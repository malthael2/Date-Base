...
void Driver::crearTabla(NombreTabla tabla, vector<NombreCampo> campos,
                        NombreCampo clave) {
Tabla t = Tabla(tabla,campos,clave);
BD.agregarTabla(tabla,t);
}

void Driver::insertarRegistro(NombreTabla& t, Registro& r) {
    BD.InsertarRegistro(t,r);
}

Respuesta Driver::consultar(const Consulta& q) {
    auto res = BD.hacerConsulta(q);
    return vector<Registro>(res.begin(), res.end());
}

BaseDeDatos Driver::verBD() {
    return BD;
}
